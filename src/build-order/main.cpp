#include <build-order/decision/weighted-dm.h>
#include <build-order/file/read-file.h>
#include <build-order/optimizers/ants.h>
#include <build-order/optimizers/knee.h>
#include <build-order/optimizers/mograsp.h>
#include <build-order/optimizers/nsga2.h>

#include <cstdlib>
#include <cstring>
#include <iostream>

using namespace BuildOrderOptimizer;

std::chrono::time_point<std::chrono::system_clock> a;
std::chrono::duration<double> tt;

State::GameState state;
Optimizers::Optimizer *optm = 0;
...
main(int argc, char const *argv[]) {
    Solution test;
    Data::Contiguous<Population> total;
    Population ret, front;
    ...
    /*
            Optimizers::OriginDM dmkr;
            Optimizers::WSM wsm;
            Optimizers::WPM wpm;
            dmkr.target = wsm.target = wpm.target = optm;

            Data::Contiguous<Data::Contiguous<double> > normalized =
       wsm.normalize(front); for (unsigned i = 0; i < normalized.size(); i++)
            {
...
set_options(int argc, char const *argv[]) {
    ...
    for (int i = 4; i < argc; i = i + 2) {
        if (!strcmp(argv[i], "-o")) {
            ...
            Optimizers::Creation::objective_minimum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::objective_maximum < Optimizers::Creation::objective_minimum)
                std::swap(Optimizers::Creation::objective_maximum, Optimizers::Creation::objective_minimum);
        } else if (!strcmp(argv[i], "-objM")) {
            Optimizers::Creation::objective_maximum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::objective_maximum < Optimizers::Creation::objective_minimum)
                std::swap(Optimizers::Creation::objective_maximum, Optimizers::Creation::objective_minimum);
        } else if (!strcmp(argv[i], "-resm")) {
            Optimizers::Creation::restriction_minimum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::restriction_maximum < Optimizers::Creation::restriction_minimum)
                std::swap(Optimizers::Creation::restriction_maximum, Optimizers::Creation::restriction_minimum);
        } else if (!strcmp(argv[i], "-resM")) {
            Optimizers::Creation::restriction_maximum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::restriction_maximum < Optimizers::Creation::restriction_minimum)
                std::swap(Optimizers::Creation::restriction_maximum, Optimizers::Creation::restriction_minimum);
        } else if (!strcmp(argv[i], "-dOm")) {
            Optimizers::Creation::delta_o_minimum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::delta_o_maximum < Optimizers::Creation::delta_o_minimum)
                std::swap(Optimizers::Creation::delta_o_maximum, Optimizers::Creation::delta_o_minimum);
        } else if (!strcmp(argv[i], "-dOM")) {
            Optimizers::Creation::delta_o_maximum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::delta_o_maximum <= Optimizers::Creation::delta_o_minimum)
                std::swap(Optimizers::Creation::delta_o_maximum, Optimizers::Creation::delta_o_minimum);
        } else if (!strcmp(argv[i], "-dRm")) {
            Optimizers::Creation::delta_r_minimum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::delta_r_maximum < Optimizers::Creation::delta_r_minimum)
                std::swap(Optimizers::Creation::delta_r_maximum, Optimizers::Creation::delta_r_minimum);
        } else if (!strcmp(argv[i], "-dRM")) {
            Optimizers::Creation::delta_r_maximum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::delta_r_maximum <= Optimizers::Creation::delta_r_minimum)
                std::swap(Optimizers::Creation::delta_r_maximum, Optimizers::Creation::delta_r_minimum);
        }

        else if (!strcmp(argv[i], "-obj")) {
            Optimizers::Creation::objective_minimum = Optimizers::Creation::objective_maximum = strtof(argv[i + 1], 0);
        } else if (!strcmp(argv[i], "-res")) {
            Optimizers::Creation::restriction_minimum = Optimizers::Creation::restriction_maximum =
                strtof(argv[i + 1], 0);
        } else if (!strcmp(argv[i], "-dO")) {
            Optimizers::Creation::delta_o_minimum = Optimizers::Creation::delta_o_maximum = strtof(argv[i + 1], 0);
        } else if (!strcmp(argv[i], "-dR")) {
            Optimizers::Creation::delta_r_minimum = Optimizers::Creation::delta_r_maximum = strtof(argv[i + 1], 0);
        }

        else if (!strcmp(argv[i], "-Fobjm")) {
            Optimizers::Fix::objective_minimum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::objective_maximum < Optimizers::Creation::objective_minimum)
                std::swap(Optimizers::Creation::objective_maximum, Optimizers::Creation::objective_minimum);
        } else if (!strcmp(argv[i], "-FobjM")) {
            Optimizers::Fix::objective_maximum = strtof(argv[i + 1], 0);
            if (Optimizers::Fix::objective_maximum < Optimizers::Fix::objective_minimum)
                std::swap(Optimizers::Creation::objective_maximum, Optimizers::Creation::objective_minimum);
        } else if (!strcmp(argv[i], "-Fresm")) {
            Optimizers::Fix::restriction_minimum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::restriction_maximum < Optimizers::Creation::restriction_minimum)
                std::swap(Optimizers::Creation::restriction_maximum, Optimizers::Creation::restriction_minimum);
        } else if (!strcmp(argv[i], "-FresM")) {
            Optimizers::Fix::restriction_maximum = strtof(argv[i + 1], 0);
            if (Optimizers::Fix::restriction_maximum < Optimizers::Fix::restriction_minimum)
                std::swap(Optimizers::Creation::restriction_maximum, Optimizers::Creation::restriction_minimum);
        } else if (!strcmp(argv[i], "-FdOm")) {
            Optimizers::Fix::delta_o_minimum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::delta_o_maximum < Optimizers::Creation::delta_o_minimum)
                std::swap(Optimizers::Creation::delta_o_maximum, Optimizers::Creation::delta_o_minimum);
        } else if (!strcmp(argv[i], "-FdOM")) {
            Optimizers::Fix::delta_o_maximum = strtof(argv[i + 1], 0);
            if (Optimizers::Fix::delta_o_maximum <= Optimizers::Fix::delta_o_minimum)
                std::swap(Optimizers::Creation::delta_o_maximum, Optimizers::Creation::delta_o_minimum);
        } else if (!strcmp(argv[i], "-FdRm")) {
            Optimizers::Fix::delta_r_minimum = strtof(argv[i + 1], 0);
            if (Optimizers::Creation::delta_r_maximum < Optimizers::Creation::delta_r_minimum)
                std::swap(Optimizers::Creation::delta_r_maximum, Optimizers::Creation::delta_r_minimum);
        } else if (!strcmp(argv[i], "-FdRM")) {
            Optimizers::Fix::delta_r_maximum = strtof(argv[i + 1], 0);
            if (Optimizers::Fix::delta_r_maximum <= Optimizers::Fix::delta_r_minimum)
                std::swap(Optimizers::Creation::delta_r_maximum, Optimizers::Creation::delta_r_minimum);
        }

        else if (!strcmp(argv[i], "-Fobj")) {
            Optimizers::Fix::objective_minimum = Optimizers::Fix::objective_maximum = strtof(argv[i + 1], 0);
        } else if (!strcmp(argv[i], "-Fres")) {
            Optimizers::Fix::restriction_minimum = Optimizers::Fix::restriction_maximum = strtof(argv[i + 1], 0);
        } else if (!strcmp(argv[i], "-FdO")) {
            Optimizers::Fix::delta_o_minimum = Optimizers::Fix::delta_o_maximum = strtof(argv[i + 1], 0);
        } else if (!strcmp(argv[i], "-FdR")) {
            Optimizers::Fix::delta_r_minimum = Optimizers::Fix::delta_r_maximum = strtof(argv[i + 1], 0);
        }
...
void
print_help(char const *program_name) {
...
void
init(char const *argv[]) {
    switch (opt_type) {
    case 0:
        optm = new Optimizers::NSGA2(archSize, creation_cycles);
        break;
    case 1:
        optm = new Optimizers::Knee(archSize, creation_cycles);
        break;
    case 2:
        optm = new Optimizers::Ants(archSize, creation_cycles, children, aco_alpha, aco_beta, aco_evrate);
        break;
    case 3:
        optm = new Optimizers::MOGRASP(archSize, creation_cycles, mograsp_parents, children);
        break;
    }

    switch (neighborhood) {
    case 0:
        optm->neighborhood = Optimizers::delete_one;
        break;
    case 1:
        optm->neighborhood = Optimizers::delete_tail;
        break;
    case 2:
        optm->neighborhood = Optimizers::one_swap;
        break;
    case 3:
        optm->neighborhood = Optimizers::insert;
        break;
    }
    ...
    /////////////////////////////////////////////////////
    a = std::chrono::system_clock::now();
    File::createRules(argv[1]);
    state = File::createState(argv[2]);
    // std::cout << "INITIAL STATE:\n";
    // state.print();
    // std::cout << "--------------\n\n";
    /////////////////////////////////////////////////////

    File::initializeOptimizer(*optm, argv[3]);
    // std::cout << optm->print();
    optm->update();
